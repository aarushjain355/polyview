...
_CONFIG_PATH = Path(__file__).parent / 'colors.yaml'
...
class VisualizationHandler:
    ...
    def make_3d_figure(self, viz_data: dict) -> go.Figure:
        ...
        fig.update_layout(
            ...
            height=720,
            legend=dict(font=dict(color='white', size=12), bgcolor='rgba(10,12,20,0.9)', bordercolor='rgba(57,255,20,0.25)', borderwidth=1),
    ...
    def add_point_cloud(self, fig: go.Figure, viz_data: dict) -> None:
        ...
        print(f'[PolyView] roi_cloud: {len(cloud)} points')
        for pt in cloud_xyz:
            print(f'  (x={pt[0]:.4f}, y={pt[1]:.4f}, z={pt[2]:.4f})')
    ...
    def render_single_lidar_metrics(self, lidar_name: str, metrics: dict) -> list[go.Figure]:
        layout = self._config['layout']
        figs = []
        for category, values in metrics.items():
            ...
            fig.update_layout(
                title=dict(text=f'<b>{category}</b>', font=dict(color='#39FF14', size=13), x=0.0),
                paper_bgcolor=layout['paper_color'],
                plot_bgcolor='rgba(8,10,18,0.0)',
                height=240,
                margin=dict(t=35, b=80, l=60, r=20),
                xaxis=dict(
                    showticklabels=True,
                    tickangle=-30,
